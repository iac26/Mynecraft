...
Trail::Trail(const float* vertex_array, unsigned long size) {
	glGenBuffers(1, &VBO);
	glGenVertexArrays(1, &VAO);

	glBindVertexArray(VAO);
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	glBufferData(GL_ARRAY_BUFFER, size, vertex_array, GL_STREAM_DRAW);
...
void Trail::redraw(const float* vertex_array, unsigned long size) {
	glBindVertexArray(VAO);
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	glBufferData(GL_ARRAY_BUFFER, size, vertex_array, GL_STREAM_DRAW);
